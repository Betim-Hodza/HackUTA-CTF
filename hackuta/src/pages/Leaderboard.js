...
const Leaderboard = () => {
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-blue-900 via-purple-900 to-black justify-center overflow-hidden">
      ...
      <h2 className="text-4xl font-bold mb-6 text-center drop-shadow-lg">🏆 Leaderboard</h2>
      <table className="w-full table-fixed border-collapse">
        <thead>
          <tr className="bg-gradient-to-r from-blue-500 to-purple-700">
            <th className="p-3 text-left font-semibold border-b border-gray-600">Rank</th>
            <th className="p-3 text-left font-semibold border-b border-gray-600">Username</th>
            <th className="p-3 text-left font-semibold border-b border-gray-600">Score</th>
          </tr>
        </thead>
        <tbody>
          {scores.map((score, index) => (
            <tr key={score.id} className={`hover:bg-purple-800 transition duration-300 ${index % 2 === 0 ? 'bg-gray-800' : 'bg-gray-900'}`}>
              <td className="p-3 text-center border-b border-gray-700">{index + 1}</td>
              <td className="p-3 border-b border-gray-700">{score.username}</td>
              <td className="p-3 text-center border-b border-gray-700">{score.score}</td>
            </tr>
          ))}
        </tbody>
      </table>
